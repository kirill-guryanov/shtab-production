  ...
  window.addEventListener("load", function () {
    ...
    window.addEventListener("scroll", () => {
      try {
        ...
        // statistics    // ыефешыешсы
        if (currentPosition >= statistics) {
          clearArrowsClasses();
...
// EXTERNAL MODULE: ./source/js/circle/circle--services.js
var circle_services = __webpack_require__(456);
...
function addInteractivityToButton_addInteractivityToButton(
  parentSelector,
  circleSvgSelector,
  circleSelector,
  left,
  ...
  if (mode === modes.bigWhiteFirst) {
    ...
    // big mode and white first
    if (mode === modes.bigWhiteFirst) {
      addInteractivityToButton_bigWhiteFirstInitialization(whiteImage, blackImage);
    }
    // +++++++++++++++++++
    // big mode and black first
    // if (mode === modes.bigBlackFirst) {
  ...
  circleSvg.addEventListener("mousemove", (event) => {
    ...
    // big mode white first
    if (mode === modes.bigWhiteFirst) {
      addInteractivityToButton_bigWhiteFirstInversion(whiteImage, blackImage);
    }
    // // big mode black first
    // if (mode === modes.bigBlackFirst) {
    //   bigBlackFirstInversion(whiteImage, blackImage)
  ...
  circleSvg.addEventListener("mouseleave", () => {
    // big mode white first
    if (mode === modes.bigWhiteFirst) {
      addInteractivityToButton_bigWhiteFirstReversion(whiteImage, blackImage);
    }
    // // big mode black first
    // if (mode === modes.bigBlackFirst) {
    //   bigBlackFirstReversion(whiteImage, blackImage)
...
// big white first utils
function addInteractivityToButton_bigWhiteFirstInitialization(white, black) {
  white.classList.add("fillingButton__image--white-visible");
  black.classList.add("fillingButton__image--black-hidden");
}
function addInteractivityToButton_bigWhiteFirstInversion(white, black) {
  white.classList.remove("fillingButton__image--white-visible");
  black.classList.remove("fillingButton__image--black-hidden");
  // colors inversions
  white.classList.add("fillingButton__image--white-hidden");
...
function addInteractivityToButton_bigWhiteFirstReversion(white, black) {
  white.classList.remove("fillingButton__image--white-hidden");
  black.classList.remove("fillingButton__image--black-visible");
  // colors inversions
  white.classList.add("fillingButton__image--white-visible");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    addInteractivityToButton_addInteractivityToButton(
...
;// CONCATENATED MODULE: ./source/js/locomotive/locomotive--services.js
...
;// CONCATENATED MODULE: ./source/js/indexes/indexServices.js
// preloader
